...
The current UI uses a sidebar-first admin shell with compact top-bar search, route-aware navigation, collapsible desktop navigation, and task-focused views for Console, Chart, Runs, RLM, Reports, and trade review. When unauthenticated, the app shows only the login screen. The design is intentionally minimal and data-dense so the live account ledger, market chart, and bridge diagnostics stay readable.
